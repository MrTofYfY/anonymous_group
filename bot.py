...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == STATE_WAIT_IMPERSONATE:
        ...
        try:
            ...
            await update.message.reply_text("Первое число должно быть anon_id.")
            return
        message_text = parts[1]
        # send as anon to all (or to non-banned users)
        for uid_k, info in DATA["users"].items():
            username_k = f"@{info['username']}" if info.get("username") else None
            if username_k in DATA["banned"]:
                continue
            # build message: Аноним#anon_id:\ntext
            try:
                await context.bot.send_message(chat_id=int(uid_k), text=f"Аноним#{anon_id}:\n{message_text}")
            except Exception as e:
                logger.warning(f"Impersonate failed for {uid_k}: {e}")
        await update.message.reply_text("✅ Сообщение отправлено от имени анонима.")
        return

    # ---- Если пользователь находится в режиме отправки (USER_SEND) ----
    user_state = context.user_data.get("state")
    if user_state == STATE_USER_SEND:
        # обычная рассылка от пользователя анонимно
        context.user_data["state"] = None
        uid_str = str(user.id)
        if DATA["users"][uid_str].get("muted_until", 0) > time.time():
            await update.message.reply_text("⏱️ Вы замьючены и не можете отправлять сообщения.")
            return
        # инкремент статистики
        DATA["message_count"] = DATA.get("message_count", 0) + 1
        save_data()
        sender_anon = DATA["users"][uid_str]["anon"]
        # если админ_chat включён и пользователь - админ, пересылать в админ-чат
        if DATA.get("admin_chat_enabled", False) and f"@{user.username}" in DATA["admins"]:
            # шлём только админам
            for a in DATA["admins"]:
                # найдём id админа
                for uid_k, info in DATA["users"].items():
                    if info.get("username") and f"@{info['username']}" == a:
                        try:
                            await context.bot.send_message(chat_id=int(uid_k), text=f"🔒 AdminChat {a}:\n{user.username or user.full_name}:\n{text}")
                        except:
                            pass
            await update.message.reply_text("✅ Сообщение отправлено в Admin Chat.")
            return
        # иначе рассылка всем (анонимно): для админов показываем их юзернеймы
        for uid_k, info in DATA["users"].items():
            if uid_k == uid_str:
                continue
            username_k = f"@{info['username']}" if info.get("username") else None
            if username_k in DATA["banned"]:
                continue
            # если получатель админ -> показывает @автор; иначе -> Аноним#id
            try:
                if username_k and username_k in DATA["admins"]:
                    # admin sees @username (of sender)
                    await context.bot.send_message(chat_id=int(uid_k), text=f"{('@' + (user.username or user.full_name))}:\n{text}")
                else:
                    await context.bot.send_message(chat_id=int(uid_k), text=f"Аноним#{sender_anon}:\n{text}")
            except Exception as e:
                logger.warning(f"Send failed to {uid_k}: {e}")
        await update.message.reply_text("✅ Сообщение отправлено всем.")
        return

    # ---- Обычное текстовое сообщение, не в состоянии ----
    # Если админ-чат включён и отправитель админ, и admin_chat_enabled, перенаправляем
    if DATA.get("admin_chat_enabled", False) and f"@{user.username}" in DATA["admins"]:
        # отправим сообщение всем админам
        for a in DATA["admins"]:
            for uid_k, info in DATA["users"].items():
                if info.get("username") and f"@{info['username']}" == a:
                    if int(uid_k) == user.id:
                        continue
                    try:
                        await context.bot.send_message(chat_id=int(uid_k), text=f"[AdminChat] @{user.username}:\n{text}")
                    except:
                        pass
        await update.message.reply_text("✅ Отправлено в Admin Chat.")
        return

    # default answer - подсказка
    await update.message.reply_text("Для отправки сообщения используйте /send или кнопку «Отправить сообщение».")
...
if __name__ == "__main__":
    # если нет админов в данных, добавим тебя (тебя нужно указать в ENV ADMIN_USER optional)
    # безопасный способ — задать основного админа через ENV: ADMIN_USER=@mellfreezy
    ADMIN_USER_ENV = os.getenv("ADMIN_USER")
    if ADMIN_USER_ENV:
        if ADMIN_USER_ENV not in DATA["admins"]:
            DATA["admins"].append(ADMIN_USER_ENV)
            DATA["permissions"].setdefault(ADMIN_USER_ENV, {p: True for p in ALL_PERMS})
            save_data()
    # гарантируем что хотя бы один админ есть (если в DATA пусто — попросим установить вручную)
    if not DATA["admins"]:
        # если нет явного админа — добавляем placeholder (тебе нужно будет заменить его через /admin после входа)
        placeholder = os.getenv("ADMIN_USER", "@mellfreezy")
        if placeholder not in DATA["admins"]:
            DATA["admins"].append(placeholder)
            DATA["permissions"].setdefault(placeholder, {p: True for p in ALL_PERMS})
            save_data()

    # старт Flask и бота в потоках
